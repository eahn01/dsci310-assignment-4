...
```{r population, echo=FALSE, fig.dim = c(8, 6), fig.cap="Horse populations for all provinces in Canada from 1906 - 1972."}
...
knitr::kable(horses_sd_table, caption = "province with highest spread in terms of horse population(in terms of standard deviation).")
```
...
See Figure \@ref(fig:horse).

```{r horse, echo=FALSE, fig.dim = c(8, 6), fig.cap="Horse populations for the province with the largest standard deviation."}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure 2 we zoom in and look at the province of 'r c, which had the largest spread of values in terms of standard deviation.
